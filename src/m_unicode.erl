...
-include("../include/m_unicode.hrl").

%% API
-export([characters_to_list/1]).
-export([characters_to_list/2]).
-export([characters_to_binary/1]).
-export([characters_to_binary/2]).
...
characters_to_list(Input) -> characters_to_list(Input, utf8).
...
characters_to_binary(Input) -> characters_to_binary(Input, utf8).
...
%% @doc Internal head trim function
...
is_unicode_alpha([]) -> true;
is_unicode_alpha([H|T]) ->
    case m_unicode_char:is_unicode_alpha(H) of
        true -> is_unicode_alpha(T);
        false -> false
    end.
...
is_ascii([]) -> true;
is_ascii([H|T]) ->
    case m_unicode_char:is_ascii(H) of
        true -> is_ascii(T);
        false -> false
    end.
...
is_ascii_alpha([]) -> true;
is_ascii_alpha([H|T]) ->
    case m_unicode_char:is_ascii_alpha(H) of
...
is_ascii_alpha_lower([H|T]) ->
    case m_unicode_char:is_ascii_alpha_lower(H) of
...
is_ascii_alpha_upper([H|T]) ->
    case m_unicode_char:is_ascii_alpha_upper(H) of
...
is_ascii_numeric([H|T]) ->
    case m_unicode_char:is_ascii_numeric(H) of
...
is_ascii_alpha_numeric([H|T]) ->
    case m_unicode_char:is_ascii_alpha_numeric(H) of
...
is_ascii_alpha_numeric_lower([H|T]) ->
    case m_unicode_char:is_ascii_alpha_numeric_lower(H) of
...
is_ascii_alpha_numeric_upper([H|T]) ->
    case m_unicode_char:is_ascii_alpha_numeric_upper(H) of
...
%% @doc Replace the specified character with the specified character or characters
...
to_upper(UnicodeList) when is_list(UnicodeList) -> [m_unicode_char:to_upper(C) || C <- UnicodeList].
to_lower(UnicodeList) when is_list(UnicodeList) -> [m_unicode_char:to_lower(C) || C <- UnicodeList].
to_title(UnicodeList) when is_binary(UnicodeList) -> [m_unicode_char:to_title(C) || C <- UnicodeList].
